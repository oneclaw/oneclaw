...
| Shell quoting in batch with `echo` | Piping JSON through `echo` can mangle quotes and special characters. Always use a heredoc (`cat <<'EOF'`) to pass batch JSON, never `echo`. |
