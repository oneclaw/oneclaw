...
34. **`chat.history` must stay off the model-catalog cold path.** `context.loadGatewayModelCatalog()` can execute provider discovery and plugin catalog hooks, so using it from a read-only history request can turn cold UI startup into a multi-minute wait. History hydration should return persisted session data and cheap config-derived defaults only; model capability inference belongs on explicit model selection or send paths.
