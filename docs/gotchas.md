...
41. **`npm run dev` does not rebuild anything — it is literally `electron .`.** There is no Vite dev server and no tsc watcher; the renderer loads `chat-ui/dist/index.html` from disk at launch. After editing source you must manually rebuild before restarting Electron:
...
43. **Never persist OneClaw-only metadata under `models.providers.*`.** openclaw 2026.4.x validates provider entries with a strict schema; fields such as `_mirroredFrom` make the gateway exit during config load with `models.providers.<id>: Unrecognized key`. If OneClaw needs normalized provider mirror entries (for example `kimi-coding` → `kimi`), the mirror must contain only schema-approved provider fields. Track/skip mirrors by comparing provider ids and entry contents in OneClaw code, or store OneClaw-only state outside `openclaw.json` (current sidecar: `~/.openclaw/oneclaw-provider-mirrors.json`).

44. **Kimi Code replay must compact old thinking signatures, not drop thinking blocks.** Kimi Code uses the `anthropic-messages` transport, and long tool-heavy sessions (notably officecli docx/pptx/xlsx builds) can accumulate large provider `thinkingSignature` payloads in historical assistant turns. Replaying those old signatures makes later requests balloon into hundreds of thousands or million-scale tokens even when visible content is small. Do **not** solve this with `dropThinkingBlocks`: Kimi rejects thinking-enabled assistant tool-call messages when `reasoning_content`/thinking is missing (`thinking is enabled but reasoning_content is missing...`). `scripts/package-resources.js#patchKimiReplayPolicy` keeps Kimi replay policy at `{ preserveSignatures: false }`, and `scripts/package-resources.js#patchKimiAnthropicThinkingSignatures` patches pi-ai's Anthropic payload conversion to replace large Kimi signatures on non-latest assistant turns with a short placeholder while keeping the thinking block shape. The latest assistant turn keeps its original signature. Both patches are intentionally fatal on anchor drift so an openclaw/pi-ai upgrade cannot silently reintroduce token blowups or reasoning_content rejection.
