...
30. **Scrollbars must use native overlay behavior.** Never customize `::-webkit-scrollbar` — it overrides the OS overlay scrollbar and makes scrollbars permanently visible. Use `scrollbar-width: thin; scrollbar-color: var(--border) transparent;` on `*` instead. This preserves the native "show on scroll, auto-hide when idle" behavior on macOS and Windows. Do not add custom `::-webkit-scrollbar-thumb` styling.
