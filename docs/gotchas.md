...
44. **Kimi Code replay must drop old thinking blocks.** Kimi Code uses the `anthropic-messages` transport, and long tool-heavy sessions (notably officecli docx/pptx/xlsx builds) can accumulate large provider `thinkingSignature` payloads in historical assistant turns. Replaying those old thinking blocks makes later requests balloon into hundreds of thousands or million-scale tokens even when the visible user content is small. `scripts/package-resources.js#patchKimiReplayPolicy` patches openclaw's bundled `KIMI_REPLAY_POLICY` to `{ preserveSignatures: false, dropThinkingBlocks: true }`, which uses openclaw's existing sanitizer to strip thinking/redacted-thinking blocks from all but the latest assistant turn. The patch is intentionally fatal on anchor drift so an openclaw upgrade cannot silently reintroduce token blowups; update the replay-policy anchor and `scripts/package-resources.test.js` if upstream changes the generated chunk shape.
