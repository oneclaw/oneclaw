...
40. **火山 DataFinder 用 server-side 端点 `gator.volces.com/v2/event/json`。** 客户端 SDK 端点 `mcs.ctobsnssdk.com` 不接 server-side payload，会返 `HTTP 400 -9 "app_id uint32 -1"`（`-1` 是错误模板里的 sentinel，不是真传了 -1）。.env 里配 `VOLCANO_ENDPOINT`，同时 `VOLCANO_APP_ID`/`VOLCANO_APP_KEY` 缺一不可，否则 `package:resources` 把 build-config.json 写空、analytics sink 被 disable。
